...
const QuotePreview: React.FC<QuotePreviewProps> = ({ data }) => {
  ...
  return (
    <div className="bg-white text-black p-8 md:p-10 shadow-2xl print-area max-w-[210mm] mx-auto min-h-[297mm] text-[11px] font-sans leading-snug relative">
      ...
      {/* Grand Total */}
      <div className="border border-black flex justify-between items-center px-2 py-2 mb-6 text-sm bg-gray-50 break-inside-avoid">
          <span className="font-bold">Total da proposta</span>
          <span className="font-bold">{formatCurrency(calculateGrandTotal())}</span>
      </div>

      {/* Conditions */}
      <div className="mb-6 text-[10px] break-inside-avoid">
          <h3 className="font-bold mb-2 text-xs">Condições de Fornecimento</h3>
      ...
      {/* Footer Notes */}
      <div className="text-[9px] text-justify leading-tight space-y-2 mb-8 break-inside-avoid">
          <div className="whitespace-pre-line">{data.notes}</div>
      </div>

      {/* Signature and Footer Block */}
      <div className="flex justify-between items-end border-t border-gray-400 pt-4 break-inside-avoid">
